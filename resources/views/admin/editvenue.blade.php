...
<body>
  <div class="main-container">
    <div class="top-nav">
      <span class="brand-title">Edit Venue</span>
      <div>
    ...
    <div class="form-wrapper">
      <div class="form-container">
        ...
        @if(session('success'))
          <div style="background: #d4edda; padding: 15px; margin-bottom: 20px; border: 1px solid #c3e6cb; color: #155724; border-radius: 5px;">
            {{ session('success') }}
          </div>
        @endif
        ...
        <form action="{{ route('venues.update', $venue->id) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            <div>
              ...
              <select id="venue-name" name="name" required>
                <option value="">-- Select Venue --</option>
                @foreach ($venues as $v)
                  <option value="{{ $v->name }}" {{ $venue->name == $v->name ? 'selected' : '' }}>{{ $v->name }}</option>
                @endforeach
              </select>
            </div>
            <div>
              <label for="venue-address">Address</label>
              <input type="text" id="venue-address" name="address" value="{{ old('address', $venue->address) }}" required>
            </div>
          </div>
          ...
          <div class="form-group">
            <div>
              ...
              <select id="city" name="city" required>
                ...
                <option value="city1" {{ $venue->city == 'city1' ? 'selected' : '' }}>City 1</option>
                <option value="city2" {{ $venue->city == 'city2' ? 'selected' : '' }}>City 2</option>
              </select>
            </div>
            <div>
              <label for="price">Price</label>
              <input type="number" id="price" name="price" value="{{ old('price', $venue->price) }}" required>
            </div>
          </div>
          ...
          <div class="form-group">
            <div>
              ...
              <input type="text" id="category" name="category" value="{{ old('category', $venue->category) }}" required>
            </div>
          </div>
          ...
          <div class="form-group">
            <div>
              ...
              <input type="number" id="capacity" name="capacity" value="{{ old('capacity', $venue->capacity) }}" required>
            </div>
            <div>
          ...
          <div class="form-group">
            <div style="flex: 1 1 100%;">
              ...
              <textarea id="description" name="description">{{ old('description', $venue->description) }}</textarea>
            </div>
          </div>
